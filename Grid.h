...
public:
	...
	Player* getPlayerByNumber(int n);
	int getNumberByPlayer(Player* player);														  // and  Draws it in the new cell
	bool Isoverlapping(GameObject* newobj);//Added
	...
	void setCurrentPlayerNumber(int a);
	...
	Ladder* GetNextLadder(const CellPosition& position);  // Gets a Pointer to the first Ladder after the passed "position"
	  // Gets a Pointer to the first PLAYER after the Current Player //Added
	void resetNextPlayers();
