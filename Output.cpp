...
int Output::GetCellStartX(const CellPosition& cellPos) const
{
	...
	return  cellX; // this line should be changed with your implementation
}
...
{
	///TODO: implement the following function as described in Output.h file
	int cellY = cellPos.VCell() * UI.CellHeight;//ADDED
	return cellY; // this line should be changed with your implementation
}
...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	pWind->GetIntegerSize(w, h, cardNum);//ADDED
	...
	y += UI.ToolBarHeight;
	pWind->DrawInteger(x, y, cardNum);//ADDED
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DESIGN_EXIT] = "images\\Menu_Exit.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\dice2.jpg";
...
void Output::PrintPlayersInfo(string info)
{
	...
	pWind->GetStringSize(w, h, info); // *ADDED*
	...
	pWind->DrawString(x, y, info); //*ADDED*
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	y += UI.CellHeight;
	pWind->DrawCircle(x, y, radius);//FILLED ADDED
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int ytmp = GetCellStartY(toCell) + 2 * UI.CellHeight;  //UI.ToolBarHeight + UI.CellHeight; // ???
	for (int i = 0; i < n; i++)
	{
		pWind->DrawLine(x12, ytmp, x34, ytmp);
		ytmp += UI.CellHeight;
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int vpx[4] = { x12,x12 + xChange,x12,x12 - xChange };
	int vpy[4] = { y1 + yChange,y1,y1 - yChange,y1 };

	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way

	pWind->DrawPolygon(vpx, vpy, 4);
